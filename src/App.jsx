...
import PostTaskSurvey from './PostTaskSurvey';
import { exportJSON, exportCSV } from './export';

// phase: 'start' | 'pre-survey' | 'task' | 'post-task' | 'summary'
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-[#f8f9fa] to-[#e9ecef] p-4 text-slate-800">
        <div className="bg-white p-10 rounded-3xl shadow-2xl max-w-md w-full border border-white relative overflow-hidden">
          ...
          <div className="w-16 h-16 bg-gradient-to-tr from-[#12887a] to-[#20b2a3] rounded-2xl flex items-center justify-center text-3xl shadow-lg shadow-[#12887a]/30 mb-8 mx-auto transform hover:rotate-6 transition-transform">
            ✨
          </div>
          ...
          <p className="text-slate-500 mb-8 text-center text-sm font-medium">
            Select the indicator condition for this participant, then begin the study.
          </p>

          <div className="flex flex-col gap-4 relative z-10">
            <button
              onClick={() => { setIndicatorType('none'); setPhase('pre-survey'); }}
            ...
            <button
              onClick={() => { setIndicatorType('numeric'); setPhase('pre-survey'); }}
            ...
            <button
              onClick={() => { setIndicatorType('qualitative'); setPhase('pre-survey'); }}
      ...
      <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-[#f8f9fa] to-[#e9ecef] p-4 py-12">
        <div className="bg-white p-10 rounded-3xl shadow-xl max-w-2xl w-full border border-white">
          <div className="text-center mb-10">
            <div className="w-16 h-16 bg-gradient-to-tr from-green-500 to-emerald-400 rounded-full flex items-center justify-center text-3xl shadow-lg shadow-green-500/30 mb-6 mx-auto">
              🎉
            </div>
            <h1 className="text-3xl font-extrabold mb-2 text-slate-800 tracking-tight">Study Completed</h1>
            <p className="text-slate-500 font-medium">
              Thank you for participating! Here is a summary of your decisions.
            </p>
            <div className="mt-3 text-xs text-slate-400 font-mono">
              Session <span className="font-bold">{sessionId}</span> · Condition:{' '}
              <span className="font-bold capitalize">
                {indicatorType === 'none' ? 'Control' : indicatorType === 'numeric' ? 'Numeric' : 'Qualitative'}
              </span>
            </div>
          </div>
          ...
          <div className="space-y-4 mb-10">
            {taskResults.map((result, idx) => (
              <div
                key={idx}
                className="p-5 border border-slate-100 rounded-2xl bg-slate-50/50 hover:bg-slate-50 transition-colors flex flex-col sm:flex-row justify-between sm:items-center gap-4"
              >
                <div>
                  <h3 className="font-bold text-slate-800 mb-1">{result.taskTitle}</h3>
                  {result.postSurvey && (
                    <div className="text-xs text-slate-400 space-x-3">
                      <span>Trust: <span className="font-semibold text-slate-600">{result.postSurvey.likert_trusted}/5</span></span>
                      <span>Accuracy: <span className="font-semibold text-slate-600">{result.postSurvey.likert_accurate}/5</span></span>
                      {result.postSurvey.likert_indicatorInfluenced != null && (
                        <span>Indicator: <span className="font-semibold text-slate-600">{result.postSurvey.likert_indicatorInfluenced}/5</span></span>
                      )}
                    </div>
                  )}
                </div>
                <span className={`px-4 py-2 text-sm font-bold rounded-lg shadow-sm w-fit ${
                  result.decision === 'Accept & Insert' ? 'bg-emerald-100 text-emerald-800 border border-emerald-200' :
                  result.decision === 'Accept & Edit' ? 'bg-sky-100 text-sky-800 border border-sky-200' :
                  result.decision === 'Reject' ? 'bg-rose-100 text-rose-800 border border-rose-200' :
            ...
            {taskResults.length === 0 && (
              <div className="p-8 text-center text-slate-400 font-medium border-2 border-dashed border-slate-200 rounded-2xl">
                Session ended early — no tasks completed.
              </div>
            )}
          </div>
          ...
          <div className="border-t border-slate-100 pt-8 space-y-4">
            <p className="text-xs font-bold text-slate-400 tracking-widest uppercase text-center mb-4">Export Session Data</p>
            <div className="flex flex-col sm:flex-row gap-3">
              <button
                onClick={() => exportJSON(sessionData)}
                className="flex-1 px-6 py-3 bg-slate-800 text-white rounded-xl font-bold hover:bg-slate-700 hover:shadow-lg transition-all duration-200 active:scale-95 shadow-md flex items-center justify-center gap-2"
              >
                ↓ Download JSON
              </button>
              <button
                onClick={() => exportCSV(sessionData)}
                className="flex-1 px-6 py-3 bg-white text-slate-700 border border-slate-300 rounded-xl font-bold hover:bg-slate-50 hover:shadow transition-all duration-200 active:scale-95 shadow-sm flex items-center justify-center gap-2"
              >
                ↓ Download CSV
              </button>
            </div>
            <div className="flex justify-center pt-2">
              <button
                ...
                className="px-8 py-3 text-slate-400 hover:text-slate-600 font-semibold transition-colors text-sm flex items-center gap-2"
              >
                ↺ Start New Session
              </button>
            </div>
      ...
      <div className="max-w-4xl w-full font-sans transition-all">
        ...
        <div className={`rounded-t-2xl px-8 py-5 flex justify-between items-center text-white shadow-md transition-colors duration-500 ${
          ...
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 bg-white/20 backdrop-blur-sm rounded-xl flex items-center justify-center text-2xl shadow-inner border border-white/20">
              ✨
            </div>
            <div>
        ...
        <div className="bg-white rounded-b-2xl shadow-2xl shadow-slate-200/50 border border-slate-200 border-t-0 p-8 sm:p-10 space-y-8 relative">
          ...
          <div className="border border-slate-200 rounded-2xl p-7 bg-slate-50/50 shadow-inner">
            ...
            <p className="text-slate-700 whitespace-pre-wrap leading-relaxed border border-slate-200 bg-white p-5 rounded-xl shadow-sm text-[16px] text-justify">
              {task.draft}
            </p>
          </div>
          ...
          <div className="border-2 border-[#e5e7eb] rounded-2xl p-7 bg-[#fcfcfc] shadow-sm relative">
            ...
            {indicatorType === 'numeric' && (
              <div className="border-2 border-emerald-200 bg-emerald-50 rounded-xl p-6 mb-8 shadow-sm transform hover:scale-[1.01] transition-transform">
                <div className="flex items-center mb-5">
                  ...
                  <span className="text-3xl font-extrabold text-emerald-600 tracking-tight drop-shadow-sm">
                    {task.confidenceNumeric}%
                  </span>
                </div>
                <div className="w-full bg-emerald-200/50 rounded-full h-3.5 mb-5 overflow-hidden shadow-inner">
                  <div
                    className="bg-gradient-to-r from-emerald-400 to-emerald-500 h-3.5 rounded-full shadow-md"
                    style={{ width: `${task.confidenceNumeric}%` }}
                  ></div>
                </div>
            ...
            {indicatorType === 'qualitative' && (
              <div className="border border-amber-400 bg-[#fff9e6] rounded-xl overflow-hidden mb-8 shadow-md transform hover:scale-[1.01] transition-transform">
                <div className="p-6 flex flex-col sm:flex-row gap-5 items-start sm:items-center">
                  <div className="flex-shrink-0 w-12 h-12 bg-gradient-to-br from-[#d57022] to-orange-600 text-white rounded-full flex items-center justify-center font-extrabold text-2xl shadow-lg ring-4 ring-orange-100">
                    !
                  </div>
                  <div>
                    <h4 className="text-[#8c4611] font-extrabold text-xl mb-1.5 tracking-tight">{task.confidenceLabel}</h4>
                    <p className="text-[#b25712] text-sm leading-relaxed font-medium">
                      The AI system is uncertain about parts of this response. Claims may be inaccurate or unverified.
                    </p>
                  </div>
                </div>
                <div className="bg-[#fff4cc] px-6 py-4 text-sm text-[#8c4611] border-t border-amber-200 gap-2 flex items-center">
                  <strong className="tracking-widest uppercase text-xs opacity-80 mr-2 border border-amber-300 py-1 px-2 rounded bg-amber-100">
                    Tip
                  </strong>
                  <span className="font-medium">
                    Review each claim carefully before accepting. Consider editing or verifying statements independently.
                  </span>
                </div>
              </div>
            )}
            ...
            <div className="flex flex-wrap justify-end gap-3 mt-4 pt-8 border-t border-slate-100">
              <button
                ...
                className="px-6 py-3 mr-auto text-slate-500 bg-white hover:bg-slate-50 rounded-xl font-bold transition-all duration-200 shadow-sm border border-slate-200 hover:border-slate-300 hover:shadow active:scale-95 flex items-center gap-2"
              >
                End Early
              </button>
              <button
                onClick={() => handleAction('Reject')}
                className="px-6 py-3 text-slate-700 bg-white hover:bg-rose-50 hover:text-rose-700 hover:border-rose-300 rounded-xl font-bold transition-all duration-200 shadow-sm border border-slate-300 hover:shadow active:scale-95"
              >
                Reject
              </button>
              <button
                onClick={() => handleAction('Accept & Edit')}
                className="px-6 py-3 text-slate-800 bg-white hover:bg-sky-50 hover:text-sky-800 hover:border-sky-300 rounded-xl font-bold transition-all duration-200 shadow-sm border border-slate-300 hover:shadow active:scale-95"
              >
                Accept & Edit
              </button>
              <button
                onClick={() => handleAction('Accept & Insert')}
                className="px-8 py-3 bg-gradient-to-b from-[#12887a] to-[#0d695e] text-white hover:from-[#0f7c6e] hover:to-[#0a564d] rounded-xl font-bold transition-all duration-200 shadow-lg shadow-teal-500/30 hover:shadow-teal-500/50 active:scale-95 border border-teal-800/20 flex items-center gap-2"
              >
                Accept & Insert
              </button>
            </div>
